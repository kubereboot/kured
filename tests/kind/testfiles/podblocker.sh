...
until ${KUBECTL_CMD:-kubectl} "${kubectl_flags[@]}" logs ds/kured --all-pods -n kube-system  | grep -i "Reboot blocked by pod"
do
  if (( attempt_num == max_attempts )); then
      echo "Attempt $attempt_num failed and there are no more attempts left!"
      exit 1
  else
      echo "Did not find 'Reboot blocked by pod' in the log, retrying in $sleep_time seconds (Attempt #$attempt_num)"
      sleep "$sleep_time"
  fi
  (( attempt_num++ ))
done